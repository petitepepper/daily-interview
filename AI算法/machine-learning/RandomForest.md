...
* 对于$t=1，2 \dots T$
  * 对训练集进行第$t$次**<font color='orange'>随机采样</font>**，共采集$m$次，得到包含$m$个样本的采样集Dt
...
**多次<font color='MediumTurquoise'>有放回的随机取样</font>，多次随机取属性**

## 3. 随机森林为什么不容易过拟合？

* 随机森林中的每一颗树都是过拟合的，拟合到非常小的细节上

* 随机森林通过<u>引入随机性</u>，使每一颗树拟合的细节不同
...
- 优点
  - 训练可以高度**并行化**，对于大数据时代的大样本训练速度有优势。个人觉得这是的最主要的优点。
  - 由于可以随机选择决策树节点划分特征，这样在样本**特征维度很高**的时候，仍然能高效的训练模型。
...
* 缺点
  * 在Adaboost训练过程中，Adaboost会使得难于分类样本的权值呈指数增长，训练将会过于偏向这类困难的样本，导致**Adaboost算法易受噪声干扰**。
